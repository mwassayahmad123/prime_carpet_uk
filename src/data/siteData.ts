export const COMPANY_NAME =
  'Prime Carpet Cleaning and Upholstery Steam Cleaning'

// Replace files in public/images/ — keep the same filenames or update paths here.
export const LOGO = {
  src: '/images/logo.svg',
  alt: 'Prime Carpet Cleaning and Upholstery Steam Cleaning',
}

export const CONTACT = {
  address: '123 Prime Street, London, UK, SW1A 1AA',
  phone: '+44 7846 364007',
  whatsapp: '+44779460958',
  email: 'hello@primecarpetcleaning.co.uk',
}

export const SERVICES = [
  {
    title: 'Carpet Steam Cleaning',
    description:
      'Deep steam cleaning removes dirt, allergens, and stains — leaving your carpets fresh, soft, and like new.',
    icon: '🧹',
  },
  {
    title: 'Upholstery Steam Cleaning',
    description:
      'Professional steam cleaning for sofas, chairs, and fabric furniture. Safe, effective, and gentle on your upholstery.',
    icon: '🛋️',
  },
  {
    title: 'Rug & Mattress Cleaning',
    description:
      'Specialist cleaning for rugs and mattresses using powerful steam extraction for a healthier home.',
    icon: '🛏️',
  },
  {
    title: 'Stain Removal',
    description:
      'Tough stain treatment for wine, pet accidents, mud, and more. We tackle what regular cleaning cannot.',
    icon: '✨',
  },
]

export const REVIEWS = [
  {
    name: 'Sarah Mitchell',
    rating: 5,
    text: 'Our carpets look brand new! The team was professional, on time, and the steam cleaning made a huge difference.',
  },
  {
    name: 'James Thompson',
    rating: 5,
    text: 'Booked a free consultation and they explained everything clearly. Highly recommend Prime Carpet Cleaning to everyone.',
  },
  {
    name: 'Emily Clarke',
    rating: 5,
    text: 'They cleaned our sofa and carpets — both look amazing. Easy booking via WhatsApp and fantastic results.',
  },
]

export const FAQ_ITEMS = [
  {
    question: 'How do I book a free consultation?',
    answer:
      'Click the "Book Free Consultation" button or fill out the contact form below. You can also reach us directly on WhatsApp for instant booking.',
  },
  {
    question: 'What services do you offer?',
    answer:
      'We specialise in carpet steam cleaning, upholstery steam cleaning, rug and mattress cleaning, and professional stain removal. Contact us for a free quote.',
  },
  {
    question: 'How long does carpet cleaning take to dry?',
    answer:
      'With our professional steam extraction, most carpets dry within 4–6 hours depending on fabric type and room ventilation.',
  },
  {
    question: 'Are your cleaning products safe for pets and children?',
    answer:
      'Yes. We use eco-friendly, non-toxic cleaning solutions that are safe for your family and pets while delivering powerful results.',
  },
  {
    question: 'Can I reschedule my appointment?',
    answer:
      'Yes, simply contact us via phone or WhatsApp at least 24 hours before your scheduled appointment and we will arrange a new time.',
  },
]

export const GALLERY_IMAGES = [
  { src: '/images/gallery/1.svg', alt: 'Freshly steam cleaned living room carpet' },
  { src: '/images/gallery/2.svg', alt: 'Upholstery steam cleaning in progress' },
  { src: '/images/gallery/3.svg', alt: 'Before and after carpet cleaning result' },
  { src: '/images/gallery/4.svg', alt: 'Professional rug steam cleaning' },
  { src: '/images/gallery/5.svg', alt: 'Clean sofa after upholstery treatment' },
  { src: '/images/gallery/6.svg', alt: 'Spotless carpet in a modern home' },
]

export const SERVICE_OPTIONS = [
  'Carpet Steam Cleaning',
  'Upholstery Steam Cleaning',
  'Rug Cleaning',
  'Mattress Cleaning',
  'Stain Removal',
  'Other',
]

export const NAV_LINKS = [
  { label: 'Home', href: '#home' },
  { label: 'About', href: '#about' },
  { label: 'Services', href: '#services' },
  { label: 'Reviews', href: '#reviews' },
  { label: 'Gallery', href: '#gallery' },
  { label: 'FAQ', href: '#faq' },
  { label: 'Contact', href: '#contact' },
]
